...
#     def __init__(self, fast_window: int = 5, slow_window: int = 20, target_notional_fraction: float = 1.0) -> None:
...
#             if len(frame) < self.slow_window:
#                 signals.append(_hold(timestamp, symbol, self.name))
#                 continue
#             fast = frame["close"].tail(self.fast_window).mean()
#             slow = frame["close"].tail(self.slow_window).mean()
#             spread = (fast - slow) / slow if slow else 0.0
#             direction = SignalDirection.LONG if spread > 0 else SignalDirection.SHORT if spread < 0 else SignalDirection.FLAT
#             signals.append(
#                 TradingSignal(
...
#                     direction=direction,
#                     strength=min(abs(spread) * 100.0, 1.0),
#                     confidence=1.0,
...
#                         feature_set=(f"ma_{self.fast_window}", f"ma_{self.slow_window}", "close"),
#                     ),
#                     confidence_metadata={"method": "deterministic_rule", "confidence_basis": "rule_trigger"},
#                     metadata={"provider": self.name, "fast": float(fast), "slow": float(slow)},
#                 )
#             )
#         return signals
    ...
    def generate(self, history: pd.DataFrame, timestamp: pd.Timestamp) -> list[TradingSignal]:
        ...
        for symbol, frame in available.groupby("symbol", sort=False):
            ...
            if len(closes) < self.window + 1:
                ...
                continue
            delta = closes.diff()
            gains = delta.clip(lower=0).rolling(self.window).mean()
            losses = (-delta.clip(upper=0)).rolling(self.window).mean()
            rs = gains.iloc[-1] / losses.iloc[-1] if losses.iloc[-1] else 0.0
            rsi = 100 - (100 / (1 + rs)) if losses.iloc[-1] else 100.0
...
def _hold(timestamp: pd.Timestamp, symbol: str, provider: str) -> TradingSignal:
    return TradingSignal(
        ...
        confidence_metadata={"method": "deterministic_rule", "confidence_basis": "insufficient_history"},
        metadata={"provider": provider, "reason": "insufficient_history"},
    )
